...
KnowYourCG (KYCG) is a CpG-centered framework designed for the
functional interpretation of DNA methylation data. Unlike existing tools
...
addresses the challenges of data sparsity in various methylation datasets,
including low-pass Nanopore sequencing, single-cell DNA methylomes,
...
## Two Ways to Access Knowledgebases

KYCG provides two methods for loading knowledgebase data:

1. **`getDBs()`** - Retrieves pre-packaged databases from ExperimentHub (RDS format)
   - Databases are cached via the sesameData package
   - Requires running `sesameDataCache()` once per installation
   - Faster loading after initial caching
   - Best for commonly-used database sets

2. **`loadDBs()`** - Loads databases directly from Zenodo or local files (TSV format)
   - Downloads tab-delimited files (.tsv or .tsv.gz) on-demand
   - No pre-caching required
   - More flexible for custom databases or latest updates
   - Can load from URLs or local files

## Available Array Knowledgebases

Array knowledgebases are available on Zenodo as listed in the following table:

| Array          | Link                                       |
|----------------|--------------------------------------------|
| Human (MSA)    | <https://zenodo.org/records/18180813>    |
| Human (EPICv2) | <https://zenodo.org/records/18176783> |
| Human (EPIC)   | <https://zenodo.org/records/18180672>   |
| Human (HM450)  | <https://zenodo.org/records/18180722>  |
| Mouse (MM285)  | <https://zenodo.org/records/18180659>  |

<img src="figures/knowledgebases.png"
     alt="Curated CpG knowledgebases" width="600"/>

## Method 1: Using getDBs() with ExperimentHub
...
databases on the local machine is important, for two reasons: it limits
the number of requests sent to the Bioconductor server, and secondly it
limits the amount of time the user needs to wait when re-downloading
database sets. For this reason, one should run `sesameDataCache()`
before loading in any database sets. This will take some time to download
all of the database sets but this only needs to be done once per
installation. During the analysis the database sets can be identified
using these accessions. knowYourCG also does some guessing when a unique
substring is given. For example, the string "MM285.designGroup" retrieves
the "KYCG.MM285.designGroup.20210210" database. Let's look at the
database group which we had used as the query (query and database are
reciprocal) in our first example:
...
can be faster thanks to the sesameData
[in-memory caching](https://tinyurl.com/2wh9tyzk), if the corresponding
database has been loaded.

## Method 2: Using loadDBs() with Zenodo TSV Files

Alternatively, you can load knowledgebases directly from Zenodo using the
`loadDBs()` function. This function accepts URLs, file paths, or directory paths
to TSV or compressed TSV files. This approach is useful for accessing the latest
database updates or working with custom databases.

Here's an example of loading a database directly from a Zenodo URL:

``` {r ky7b, eval=FALSE}
# Load ImprintingDMR database from Zenodo
dbs_imprinting <- loadDBs(
  "https://zenodo.org/records/18176501/files/ImprintingDMR.20220818.gz")

# Examine the loaded database
length(dbs_imprinting)  # Number of databases
names(dbs_imprinting)   # Database names
head(dbs_imprinting[[1]])  # Preview first database
```

You can also load multiple databases at once by providing multiple URLs:

``` {r ky7c, eval=FALSE}
# Load multiple databases from Zenodo
urls <- c(
  "https://zenodo.org/records/18176501/files/ImprintingDMR.20220818.gz",
  "https://zenodo.org/records/18176501/files/Blacklist.20220304.gz"
)
dbs_multiple <- loadDBs(urls)

# Use these databases in enrichment testing
results <- testEnrichment(query, dbs_multiple, platform="HM450")
```

The databases loaded via `loadDBs()` can be used directly in `testEnrichment()`
just like those loaded via `getDBs()`.

# ENRICHMENT TESTING

The main work horse function for testing enrichment of a categorical query
against categorical databases is the `testEnrichment` function. This
function will perform Fisher's exact testing of the query against each
database set (one-tailed by default, but two-tailed optionally) and
reports overlap and enrichment statistics. 
...
`linkProbesToProximalGenes` function, which internally utilizes
[g:Profiler2](https://biit.cs.ut.ee/gprofiler/gost) for the enrichment
analysis: 

```{r ky12, message=FALSE, eval=FALSE}

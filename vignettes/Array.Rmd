...
1. **`getDBs()`** - Retrieves pre-packaged databases from ExperimentHub
...
2. **`readRDS()`** - Loads databases directly from Zenodo or local files
   - Downloads RDS files
...
| Human (EPIC)   | <https://zenodo.org/records/18180671> |
| Human (HM450)  | <https://zenodo.org/records/18176500> |
| Mouse (MM285)  | <https://zenodo.org/records/18180658> |

<img src="figures/knowledgebases.png"
     alt="Curated CpG knowledgebases" width="600"/>

## Method 1: ExperimentHub (getDBs)
...
## Method 2: Zenodo RDS files

Alternatively, you can load knowledgebases directly from Zenodo using the
`readRDS()` function. This function accepts URLs, file paths, or directory
paths to RDS files. This approach is useful for accessing the latest database
updates or working with custom databases.

Here's an example of loading a database directly from a Zenodo URL:

``` {r ky7b, eval=FALSE}
# Load ImprintingDMR database from Zenodo
dbs_imprinting <- readRDS(url(paste0(
    "https://zenodo.org/records/18344496/files/",
    "KYCG.HM450.ImprintingDMR.20220818.rds")))
...
urls <- paste0(
    "https://zenodo.org/records/18344496/files/",
    c("KYCG.HM450.ImprintingDMR.20220818.rds",
        "KYCG.HM450.Blacklist.20220304.rds"))

dbs_multiple <- do.call(c, lapply(urls, function(x) readRDS(url(x))))
...
The databases loaded via `readRDS()` can be used directly in `testEnrichment()`

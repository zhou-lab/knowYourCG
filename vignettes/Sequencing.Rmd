...
For sequencing data at scale, KnowYourCG v2
([kycg](https://github.com/zhou-lab/kycg)) runs this same analysis from the
command line, natively over YAME `.cg` stores and fetching its own
knowledgebases.

The chunk below is **shown but not run**: it downloads roughly 12 MB from
Zenodo, and a vignette that reaches the network at build time fails whenever
the transfer is slower than R's download timeout, which has no bearing on
whether the package works. Copy it to run it.
...
| Assembly     | Link                                    |
|--------------|-----------------------------------------|
| human (hg38) | <https://doi.org/10.5281/zenodo.18175837> |
| mouse (mm10) | <https://doi.org/10.5281/zenodo.18175655> |
...
Then we simply run
[`yame summary`](https://zhou-lab.github.io/YAME/)
...
[`yame pack`](https://zhou-lab.github.io/YAME/).

...
A tool for automated functional enrichment analysis of DNA methylation data

# Quick Start

knowYourCG is a tool for evaluating the enrichment of CpG probes in different
methylation feature sets. These categories or features can be categorical 
(e.g., CpGs located at specific tissue-specific transcription factors) or continuous 
(e.g., the local CpG density at a regulatory element). Additionally, the set of CpGs 
to which the test will be applied can be categorical or continuous as well.
...
epigenome-wide association study. We have curated a variety of database sets that 
represent different categorical and continuous methylation features such as CpGs 
associated with particular chromatin states, technical artifacts, gene association and
gene expression correlation, transcription factor binding sites, tissue specific
methylation, CpG density, etc.
...
The main work horse function for testing enrichment of a categorical query against
categorical databases is the `testEnrichment` function. This function will perform 
Fisher's exact testing of the query against each database set 
...
The output of each test contains multiple variables including: the estimate (fold
enrichment), p-value, overlap statistics, type of test,
as well as the name of the database set and the database group. By default, the 
results are sorted by -log10 of the of p-value and the fold enrichment.
...
database sets can be identified using these accessions. knowYourCG also does some
...
# Retrieving more database sets

Some commonly used database sets are stored in ExperimentHub/sesameData package. 
But more database sets can be found 
[here](https://github.com/zhou-lab/KYCG_knowledgebase_EPICv2). You can download 
these database sets to a local folder and easily load them into memory:

```{r eval=FALSE}
KYCG_listDBGroups(path = "~/Downloads/KYCG_knowledgebase_EPICv2/hg38/")
...
dbs <- KYCG_loadDBs("~/Downloads/KYCG_knowledgebase_EPICv2/hg38/")
...
results[,c("dbname","gene_name","estimate","FDR", "nQ", "nD", "overlap")] %>% head()
```


# GO/Pathway Enrichment

One can get all the genes associated with a probe set and test the Gene Ontology of 
the probe-associated genes using the ```testGO()``` function, which internally utilizes
[g:Profiler2](https://biit.cs.ut.ee/gprofiler/gost) for the enrichment analysis: 
...
Sometimes it may be of interest whether a query set of probes share close genomic 
proximity. Co-localization may suggest co-regulation or co-occupancy in the same regulatory element. KYCG can test for genomic proximity using the ```testProbeProximity()```
function. Poisson statistics for the expected # of co-localized hits from the given query size (lambda) and the actual co-localized CpG pairs along with the p value are returned: 
...
CpG modules can be identified by passing a betas matrix to the ```findCpGModules()```
function. This will return a list of vectors, each of which contains highly correlated
CpG probes:
...
To link co-methylated CpGs to potential biology, one can test the enrichment of the modules:
...
We see modules that are enriched in sex, tissue specific methylation, SNPs, transcription
factor binding sites and other genomic features

# Missing Data Imputation

One can impute missing values in a betas matrix using the correlation network workflow. Highly correlated
CpGs can be imputed with greater accuracy compared to traditional mean based imputation. If a 
missing probe does not have a co-methylated neighbor in the data, then the mean is used. This hybrid approach
improves overall imputation accuracy and is especially effective at imputing values that show variation across
the data and thus may have biological significance
...
Note: imputation can be time and memory consuming if the data set is large or 
the graphSize parameter is too high

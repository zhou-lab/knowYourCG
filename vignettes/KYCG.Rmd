...
We can visualize the result of this test using the `plotEnrichAll`
function:

```{r ky4, fig.width=7, fig.height=4.5,message=FALSE}
plotEnrichAll(results_pgc)
```
...
plotLollipop(results, label="gene_name")
```
...
followed by calling the `plotSetEnrichment()` function.

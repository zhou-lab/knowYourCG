...
An automated tool for discovering hidden biological and technical
links in DNA methylation data

# Quick Start

knowYourCG is a tool for evaluating the enrichment of CpG probes in different
methylation feature sets. This tool automates hypothesis testing by asking 
whether a set of CpGs (indexed by Illumina methylation chip IDs, hence a sparse representation
...
density at a regulatory element). Additionally, the set of CpGs to which the test will be
...
methylation analysis or an epigenome-wide association study. We have taken
the time to curate our own database sets from a variety of sources that
describe different categorical and continuous features such as CpGs associated 
with certain chromatin states, technical artifacts, gene association, 
transcription factor binding sites, CpG density, etc.
...
calculates the extent of overlap and applies different statistics for enrichment
...
enrichment, not the test statistics), p-value, overlap statistics, type of test,

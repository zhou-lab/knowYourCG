...
#'
#' @examples
#' data=list(estimate=c(runif(10)), p.value=c(runif(10)))
#' plotVolcano(data)
#'
#' @export
plotVolcano = function(data, title=NA, xlabel=NA, ylabel=NA) {
    ...
    data = data.frame(
        estimate=log(data$estimate + 1, base=10),
        p.value=-log(data$p.value + 1, base=2)
        )
    ...
    if (any(data$p.value > -log(0.05))) {
        g = ggplot(data=data,
                   aes(
                       x=estimate,
                       y=p.value,
                       color = cut(p.value, c(-log(0.05), Inf))))
    } else {
    ...
    # values = c("[2.995732, Inf)" = "red"), labels = c("> 2.995732"))
    # dev.off(0)
    g
}
